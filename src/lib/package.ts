...
const gitignore = [
  ...
  '.DS_STORE',
  '',
  '# build, dependencies, coverage and test outputs',
  'out',
  'dist',
  'coverage',
  'node_modules',
...
const npmrc = `//localhost:2092/:_authToken="dummy-token"\n@drpm:registry=http://localhost:2092\n`;
...
export class Package {
  ...
  static async init({
    ...
    await writeFile(`${name}/.npmrc`, npmrc);
    ...
    await writeFile(`${name}/.gitignore`, gitignore.join('\n'));
