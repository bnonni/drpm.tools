import { DwnInterface, DwnMessageDescriptor } from '@web5/agent';
...
type DwnRecordDescriptor = DwnMessageDescriptor[DwnInterface.RecordsWrite] | DwnMessageDescriptor[DwnInterface.RecordsDelete];
...
export async function fetchResource(did: string, name: string, version: string): Promise<string> {
  ...
  for (const endpoint of endpoints) {
    ...
    const { status: qStatus, entries: qEntries } = await query.json() as DRLResponse;
